...
def find_amoc_files(experiment: str, input_dir: str, config: dict) -> list:
    """
    Find AMOC diagnostic files for the given experiment.
    ...
    -------
    list
        Sorted list of AMOC diagnostic file paths
        
    Raises
    ------
    FileNotFoundError
        If no AMOC files are found for the experiment
    """
    ...
    file_pattern = config['file_patterns']['amoc'].format(exp=experiment)
    ...
    if not files:
        raise FileNotFoundError(
            f"No AMOC files found for experiment '{experiment}' "
...
def amoc_plot(experiment: str, files: list, nyear: int, config: dict) -> plt.Figure:
    """
    Create AMOC mean section plot.
    
    This function generates a contour plot of the mean Atlantic Meridional
    Overturning Circulation (AMOC) over the last N years of the simulation.
    ...
    nyear : int
        Number of years to average (from the end of the simulation)
    config : dict
    ...
    section_config = plot_config['section_plot']
    ...
    try:
        ...
        amoc = data['msftyz'].compute()
        
        # Calculate the start index for averaging
        total_time_steps = len(amoc.time_counter)
        start_idx = max(0, total_time_steps - nyear) if total_time_steps > nyear else 0
        
        # Compute mean over the last nyear years
        amoc_mean = amoc.isel(time_counter=slice(start_idx, -1)).mean('time_counter')
        amoc_mean = amoc_mean.squeeze()
        
        # Create contour plot for Atlantic basin
        amoc_mean.sel(basin=section_config['basin']).plot.contourf(
            x='nav_lat', 
            y='depthw',
            ylim=section_config['ylim'],
            xlim=section_config['xlim'],
            levels=section_config['contour_levels']
        )
        
        plt.title(f'AMOC {experiment} (Sv) mean last {nyear} years')
        plt.xlabel('Latitude')
        plt.ylabel('Depth (m)')
        
    except Exception as e:
        raise ValueError(f"Error creating AMOC section plot: {e}")
...
def main():
    ...
    This function orchestrates the entire AMOC analysis workflow:
    ...
    try:
        ...
        # Find AMOC files
        print(f"Looking for AMOC files for experiment '{args.experiment}'...")
        amoc_files = find_amoc_files(args.experiment, input_dir, config)
        print(f"Found {len(amoc_files)} AMOC files")
        
        # Generate plots based on user selection
        if not args.timeseries_only:
            print(f"Generating AMOC section plot (averaging last {nyear} years)...")
            fig_section = amoc_plot(args.experiment, amoc_files, nyear, config)
            save_figure(fig_section, f'amoc_section_{args.experiment}', output_dir)
        ...
        print("AMOC analysis completed successfully!")
